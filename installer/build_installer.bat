...
@echo ==================================================
@echo Building HlaeFfmpegMigrateExtension ...
@echo ==================================================
mkdir "%~1\HlaeFfmpegMigrateExtension"
cd "%~1\HlaeFfmpegMigrateExtension"
cd "%~dp0HlaeFfmpegMigrateExtension"
"%~3" -t:build -restore "%~dp0HlaeFfmpegMigrateExtension\HlaeFfmpegMigrateExtension.csproj" "-property:Configuration=Release" "-property:Platform=x86" "-property:OutputPath=%~1\HlaeFfmpegMigrateExtension"
@IF !errorlevel! NEQ 0 EXIT /B 1
...
@echo ==================================================
@echo Building HlaeFfmpegMigrate ...
@echo ==================================================
@REM
@REM The Output as HlaeFmpeg.msi is intentional, see https://stackoverflow.com/questions/75918271/cant-use-msi-after-renaming-it
@REM
@FOR %%l IN (en-US de-DE fi-FI hu-HU it-IT ja-JP nl-NL pl-PL pt-BR pt-PT ru-RU tr-TR zh-CN) DO (
    mkdir "%~1\HlaeFfmpegMigrate\%%l"
    mkdir "%~1\HlaeFfmpegMigrate\%%l\tmp"
    cd "%~1\HlaeFfmpegMigrate\%%l"
    wix build -arch x86 -defaultcompressionlevel high -culture "%%l" -intermediateFolder "%~1\HlaeFfmpegMigrate\%%l\tmp" -o "%~1\HlaeFfmpegMigrate\%%l\HlaeFfmpeg.msi" -ext WixToolset.UI.wixext/5.0.2 -d "var.HlaeFfmpegMigrateExtension.TargetDir=%~1\HlaeFfmpegMigrateExtension" "%~dp0HlaeFfmpegMigrate\Package.wxs" "%~dp0HlaeFfmpegMigrate\MyWixUI_InstallDir.wxs" "%~dp0HlaeFfmpegMigrate\lang\%%l.wxl" "%~dp0shared\Dependency\lang\%%l.wxl"
    @IF !errorlevel! NEQ 0 EXIT /B 1
    wix msi validate "%~1\HlaeFfmpegMigrate\%%l\HlaeFfmpeg.msi"
    @IF !errorlevel! NEQ 0 EXIT /B 1
)
cd "%~1\HlaeFfmpegMigrate"
@FOR %%l IN (de-DE fi-FI hu-HU it-IT ja-JP nl-NL pl-PL pt-BR pt-PT ru-RU tr-TR zh-CN) DO (
    wix msi transform -t language -intermediateFolder "%~1\HlaeFfmpegMigrate\%%l\tmp" "%~1\HlaeFfmpegMigrate\en-US\HlaeFfmpeg.msi" "%~1\HlaeFfmpegMigrate\%%l\HlaeFfmpeg.msi" -o "%~1\HlaeFfmpegMigrate\%%l\HlaeFfmpeg.mst"
    @IF !errorlevel! NEQ 0 EXIT /B 1
)
